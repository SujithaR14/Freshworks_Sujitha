...
int main()
{
    // New instance of our data store suji is created by heap allocation.
    suji * taskPtr = new suji();
    ...
    // here we use multithreading concept to make sure that it can be optimally used when multiple clients are working on our api program.
     taskPtr->create("sujitha","Tele : 8606885746",4);
    ...
    std::thread th1(&suji::create, taskPtr, "sujitha","message : Hi iam a bTech graduate",7); // Creating using thread.
    Sleep(10000);
    std::thread th2(&suji::create, taskPtr, "gif","dhbcsdhb",0);
    ...
    std::thread th3(&suji::_delete, taskPtr, "gif"); // deleting a key-value pair using thread.
    th3.join();
    th1.join();
    th2.join();
    ...
    cout<<taskPtr->read("sujitha")<<endl;
    ...
    delete taskPtr;// the heap allocation of the instance of the data store suji is deleted.
    return 0;
}